...
class tqdm {
    private:
        ...
        void hsv_to_rgb(float h, float s, float v, size_t& r, size_t& g, size_t& b) {
            if (s < 1e-6) {
                v *= 255.;
                r = v; g = v; b = v;
            }
            size_t i = (size_t)(h*6.0);
            float f = (h*6.)-i;
            size_t p = (size_t)(255.0*(v*(1.-s)));
            size_t q = (size_t)(255.0*(v*(1.-s*f)));
            size_t t = (size_t)(255.0*(v*(1.-s*(1.-f))));
            v *= 255;
            i %= 6;
            size_t vi = (size_t)v;
            if (i == 0)      { r = vi; g = t;  b = p;  }
    ...
    public:
        ...
        void progress(size_t curr, size_t tot) {
            if(is_tty && (curr%period == 0)) {
                ...
                if (use_ema) {
                    ...
                    size_t dnsum = std::accumulate(deq_n.begin(),deq_n.end(),0.);
                    avgrate = dnsum/dtsum;
                }
                ...
                if (nupdates > 10) {
                    period = (size_t)( std::min(std::max((1.0/25)*curr/dt_tot,1.0), 5e5));
                    smoothing = 25*3;
                }
                ...
                for (size_t i = 0; i < ifills; i++) std::cout << bars[8];
                if (!in_screen and (curr != tot)) printf("%s",bars[(int)(8.0*(fills-ifills))]);
                for (size_t i = 0; i < width-ifills-1; i++) std::cout << bars[0];
                ...
                printf("[%4lld/%4lld | %3.1f %s | %.0fs<%.0fs] ", curr,tot,  avgrate/div, unit.c_str(), dt_tot, peta);
